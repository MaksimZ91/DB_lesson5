...
CREATE OR REPLACE VIEW frends
AS
SELECT id, concat(firstname, " ", lastname) AS "Имя Фамилия"
FROM friend_requests
JOIN users ON (initiator_user_id = 1 OR target_user_id = 1) ;
...
CREATE OR REPLACE VIEW frends
AS
SELECT users.id, CONCAT(users.firstname, " ", users.lastname) AS "Имя Фамилия"
FROM (
	SELECT target_user_id, initiator_user_id, `status`
    FROM friend_requests
    WHERE ( target_user_id = 1 OR initiator_user_id = 1 ) AND `status` = 'approved') fr
JOIN users 
ON ( initiator_user_id = users.id OR users.id = target_user_id ) AND users.id != 1;

SELECT * from frends;

-- Создайте представление, в котором будут выводится все сообщения, в которых принимал
-- участие пользователь с id = 1.
CREATE OR REPLACE VIEW mss
AS
...
CREATE OR REPLACE VIEW likes_for_file
AS
SELECT u.id AS "userID" , m.id AS "mediaID", m.filename, `filelikes`
FROM media m
JOIN users u ON u.id = m.user_id
LEFT JOIN (	
			SELECT media_id, COUNT(user_id) as `filelikes`
            FROM  likes
            GROUP BY media_id
            ) AS lk
	ON m.id = lk.media_id
...
SELECT * FROM likes_for_file;

-- Получите количество групп у пользователей
SELECT	u.id AS "userID", 
		CONCAT(firstname, " ", lastname) AS "Имя Фамилия", 
		`group_count` AS "Кол-во групп"   
FROM users u
LEFT JOIN (
			SELECT COUNT(community_id) as `group_count` , user_id
			FROM users_communities 
			GROUP BY  user_id
            ) AS ucomm
	ON ucomm.user_id = u.id;   
    
-- Вывести 3 пользователей с наибольшим количеством лайков за медиафайлы
	-- работаем с результатом VIEW likes_for_file
    
SELECT userID, 
		CONCAT(firstname, " ", lastname) AS "Имя Фамилия", 
		SUM(filelikes) summ_likes
FROM likes_for_file   
